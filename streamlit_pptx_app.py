# Streamlit app to generate and download a PowerPoint slide deck for Cloud Security Transformation with CCSP
# Prerequisites: Install streamlit and python-pptx (pip install streamlit python-pptx)
...
try:
    ...
    st.error("The 'python-pptx' library is not installed. Install it by running: `pip install python-pptx`")
    st.error("If using Python 3, try: `pip3 install python-pptx`")
    st.error("For virtual environments, activate the environment first.")
    st.error("For permission issues, try: `pip install python-pptx --user`")
    st.stop()
...
def generate_pptx():
    ...
    for i, series in enumerate(chart.series):
        for point in series.points:
            point.fill.solid()
            point.fill.fore_color.rgb = [PRIMARY_BLUE, ACCENT_RED, GREEN, PURPLE][i]
